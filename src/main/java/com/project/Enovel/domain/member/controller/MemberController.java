...
public class MemberController {
    ...
    public String signup(MemberCreateForm memberCreateForm, BindingResult bindingResult) {
        ...
        try {
            memberService.create(memberCreateForm.getUsername(), memberCreateForm.getPassword1(), memberCreateForm.getNickname(),
                    memberCreateForm.getEmail(), memberCreateForm.getAddress(), memberCreateForm.getPhone(), false, false, false);
    ...
    public String myPage(Model model, MemberCreateForm memberCreateForm, MemberMyPageForm memberMyPageForm,Principal principal) {
        String username = principal.getName();
        Member member = this.memberService.getMember(username);
        if (!member.getUsername().equals(principal.getName())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "수정권한이 없습니다.");
        ...
        model.addAttribute("memberMyPageForm", memberMyPageForm);
